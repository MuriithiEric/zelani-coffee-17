...
export default function Admin() {
  ...
  useEffect(() => {
    loadOrders();
    // Load products
    const storedProds = localStorage.getItem("zelani_products");
    if (storedProds) {
      setProducts(JSON.parse(storedProds));
    } else {
      setProducts(initialProducts);
      localStorage.setItem("zelani_products", JSON.stringify(initialProducts));
    }
    // Load users
    const storedUsers = localStorage.getItem("zelani_users");
    if (storedUsers) {
      setUsers(JSON.parse(storedUsers));
    } else {
      setUsers(initialUsers);
      localStorage.setItem("zelani_users", JSON.stringify(initialUsers));
    }
  }, []);
  ...
  const handleAddProduct = (e: React.FormEvent) => {
    ...
    setProducts(updated);
    localStorage.setItem("zelani_products", JSON.stringify(updated));
    closeModal();
  ...
  const handleUpdateUserRole = () => {
    const updated = users.map((u) =>
      u.email === modalEmail ? { ...u, role: selectedUserRole } : u
    );
    setUsers(updated);
    localStorage.setItem("zelani_users", JSON.stringify(updated));
    closeModal();
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-zinc-100 text-sm font-medium">
                  {orders.length === 0 ? (
                    ...
                    orders.map((o) => (
                      <tr key={o.id} className="hover:bg-zinc-50/50 transition-colors">
                        <td className="p-4 pl-6 text-zinc-500 text-xs">{new Date(o.created_at).toLocaleString()}</td>
                        <td className="p-4 font-bold text-zinc-800">{o.customer_email || "Anonymous Guest"}</td>
                        <td className="p-4">
                          <span className="flex items-center gap-2 text-zinc-650 font-semibold">
                            <CreditCard className="h-4 w-4 text-[#b37e38]" />
                            {o.customer_phone?.startsWith("254") ? "M-Pesa Checkout" : "Card Checkout"}
                          </span>
                        </td>
                        <td className="p-4 font-mono text-xs text-zinc-500">{o.order_reference}</td>
                        <td className="p-4 text-right pr-6 space-x-1">
                          <button
                            onClick={() => handleShareLink("payment", o.id)}
                            className="p-2 text-zinc-450 hover:text-zinc-650 transition-colors hover:bg-zinc-100 rounded-full"
                          ...
                          <button
                            onClick={() => setSearchParams({ modal: "payment", id: o.id })}
                ...
                <tbody className="divide-y divide-zinc-100 text-sm font-medium">
                  {users.map((u, index) => (
                    <tr key={index} className="hover:bg-zinc-50/50 transition-colors">
                      ...
                      <td className="p-4 text-zinc-505 text-xs">{u.joined}</td>
